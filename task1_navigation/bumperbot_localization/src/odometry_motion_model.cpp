...
void OdometryMotionModel::odomCallback(const nav_msgs::msg::Odometry &odom)
{
    ...
    double rot1_variance = alpha1_ * delta_rot1 * delta_rot1 + alpha2_ * delta_trans;
    double trans_variance = alpha3_ * delta_trans * delta_trans + alpha4_ * (delta_rot1 * delta_rot2);
    ...
    std::normal_distribution<double> transl_noise(0.0, trans_variance);
    ...
    for(auto & sample : samples_.poses)
    {
        ...
        double delta_transl_draw = delta_trans + transl_noise(noise_generator);
